DROP DATABASE IF EXISTS vk_vk;
CREATE DATABASE vk_vk;
USE vk_vk;
...
DROP PROCEDURE IF EXISTS RandomUser;
DELIMITER //
CREATE PROCEDURE RandomUser(
    IN userId INT) -- входной целочисленный параметр
BEGIN
    DECLARE city VARCHAR(100);
    DECLARE groupId INT;
    
    -- Получаем город и группу для данного пользователя
    SELECT hometown INTO city FROM `profiles` WHERE user_id = userId;
    SELECT community_id INTO groupId FROM users_communities WHERE user_id = userId LIMIT 1;
    
    -- Выбираем 5 случайных пользователей на основе наших критериев
    SELECT u.id, u.firstname, u.lastname, p.hometown, uc.communities
    FROM users AS u
    LEFT JOIN `profiles` AS p ON u.id = p.user_id
    LEFT JOIN (
        SELECT user_id, GROUP_CONCAT(community_id) AS communities
        FROM users_communities
        GROUP BY user_id
    ) AS uc ON u.id = uc.user_id
    WHERE u.firstname <> (SELECT firstname FROM users WHERE id = userId)
        AND (
            p.hometown = city
            OR EXISTS (SELECT * FROM users_communities WHERE user_id = u.id AND community_id = groupId)
            OR EXISTS (
                SELECT *
                FROM friend_requests AS f1
                JOIN friend_requests AS f2 ON f1.target_user_id = f2.initiator_user_id
                WHERE f1.initiator_user_id = userId AND f2.target_user_id = u.id
            )
        )
    ORDER BY RAND() -- позволяет получить рандомных пользователей, иначе будут идти по порядку
    LIMIT 5;
END //
